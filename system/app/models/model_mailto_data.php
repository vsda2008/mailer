...
class Model_mailto_data extends Model{
	 ...
	 /* public function test_send($id,$subject='test subject',$html_content='邮件测试',$replyto='',$to='',$from='auto',$word_wrap=50,$altbody='您的邮箱不支持html!',$replyto_name='旅途中国',$from_name='旅途中国'){
			...
			$from="648896840@qq.com";
			$this->mailer->AddAddress($from,'阳光之路');
			$re = $this->mailer->send();
			if( ! $re){
	  ...
	  public function send($limi22,$temp_id11,$subject,$html_content,$replyto,$to,$tid,$from='auto',$word_wrap=50,$altbody='您的邮箱不支持html!',$replyto_name='旅途中国',$from_name='旅途中国'){
		  static $i;
		  $i++;
		  $this->mailer->ClearAddresses();
			...
			if($re){
				$data['send_is']=1;
				$this->db->update(EMAIL_TABLE, $data, "id = $tid");
				return true;
			}
			else{
			return false;
	  ...
	  function get_mail(){
	  $sql="select email_addr  from email";
	  $q=$this->db->query($sql);
	  if($q->num_rows() >0){
		   $type = $q->result_array();
		  return $type;
	  ...
	  function clear($q){
		$sql="select *  from email where email_addr='$q'";
		$q=$this->db->query($sql);
	  if($q->num_rows() >1){
		   $type = $q->result_array();
		   foreach($type as $type){
		   $this->db->update('email', array('email_addr' =>'648896840@qq.com'), array('id' => $type['id']));}
	   }
	  }
	  ...
	  function get_hit($q=0){
		$query = $this->db->get('f_hit');
		$query = $this->db->get_where('f_hit', array('up_f' => $q));
	  ...
	  function get_hr($q){
		  if($q){
				foreach($q as $s){
				$query = $this->db->get_where('hit', array('hit_url' => $s['f_id']));
				$hr1['num']=$query->num_rows();
				$hr1['f_name']=$s['f_name'];
				$hr[]=$hr1;
				}
		  }
