...
class Model_customers extends Model{
	...
	function add_type($tye,$isn,$array){
	   $sql = 'SELECT `res_type` FROM `'.C_CATESYS_TABLE."` WHERE `tid` = $tye LIMIT 1";
	   $q = $this->db->query($sql);
	   if($q->num_rows() >0){
		   $type = $q->result_array();
		   $type = $type[0];
	...
	function get_type($tye){
	   $sql = 'SELECT `tname`,`res_type` FROM `'.C_CATESYS_TABLE."` WHERE `tid` = $tye LIMIT 1";
	   $q = $this->db->query($sql);
	   if($q->num_rows() >0){
		   $type = $q->result_array();
		   $type = $type[0];
	...
	function add_aplink($array){
		$this->db->insert(C_TLINK_TABLE,$array);
		if($this->db->insert_id() > 0){
			echo '添加成功！';
		}else{
		    echo '添加失败！';
	...
	function mod_aplink($lid,$array){
		$this->db->where('lid',intval($lid));
		$this->db->update(C_TLINK_TABLE,$array);
		if($this->db->affected_rows() > 0){
			return true;
		}
	}
	function add_issn($isn_name){
	     ...
	     $this->db->insert(CUSTOMERS_TABLE,$fiedls);
		 if($this->db->insert_id() > 0){
			 return true;
		 }
	}
	...
	function get_issn($id){
		$q = $this->db->get_where(CUSTOMERS_TABLE,array('id'=>$id),1,0);
	...
	function get_aplink($lid){
		$q = $this->db->get_where(C_TLINK_TABLE,array('lid'=>$lid),1,0);
		$result = $q->result_array();
		if($q->num_rows() > 0){
			return $result[0];
		}else{
	...
	function get_public($id,$issn){
		$q = $this->db->get_where(C_TLINK_TABLE,array('tid'=>$id,'issn'=>$issn),1,0);
		$result = $q->result_array();
		if($q->num_rows() > 0){
			return $result[0];
		}else{
	...
	function get_hd($id,$issn){
		$q = $this->db->get_where(C_TLINK_TABLE,array('tid'=>$id,'issn'=>$issn),6,0);
		if($q->num_rows() > 0){
           foreach( $q->result_array() as $v ){
			    $all_hd[] = $v;
	...
	function del_aplink($tid,$lids){
	      $ids = implode(',',$lids);
		  $sql = 'DELETE FROM `'.C_TLINK_TABLE.'` WHERE `lid` in('.$ids.')';
		  $q = $this->db->query($sql);
		  if( $this->db->affected_rows() > 0){
			  return true;
		  }
	...
	function select_user($user,$pass){
		...
		if($res){
			...
			if($res==$input){
				session_start();
				$_SESSION['user'] = $user;
			return"<script>alert('登录陈功');document.location.href='/index.php/welcome/main';</script>";
			}else{
