	  ...
	  public function send($limi22,$temp_id11,$subject,$html_content,$replyto,$to,$tid,$from='auto',$word_wrap=50,$altbody='您的邮箱不支持html!',$replyto_name='旅途中国',$from_name='旅途中国'){
		  $this->mailer->ClearAddresses();
			...
			if($send_type === 'mx'){
			  ...
			  echo "已发送".$limi22."封邮件发送HOST:".$smtp_info['host']."发送帐号：".$smtp_info['username']."收件人:".$to."<br/>";
			  $this->mailer->SMTPAuth = (boolean)$smtp_info['is_auth'];
			  $this->mailer->Host =  $smtp_info['host'];
			  $this->mailer->Username =  $smtp_info['username'];
			  $this->mailer->Password =  $smtp_info['pwd'];
			...
			var_dump("fdfdf");
			if($re){
				//echo "<script>document.location.href='/index.php/mailer/do_bat_mail/li/".++$limi22."/temp_id/".$temp_id11."';</script>";
			}else
				{
			   echo $this->mailer->ErrorInfo;
			   //echo "<script>document.location.href='/index.php/mailer/do_bat_mail/li/".++$limi22."/temp_id/".$temp_id11."';</script>";
			   log_message('debug','fail '.$to.$this->mailer->ErrorInfo.$limi22.$smtp_info['host'].$smtp_info['username']);
			   //echo "<script>document.location.href='/index.php/mailer/do_bat_mail/li/".++$limi22."/temp_id/".$temp_id11."';</script>";
			   return false;
			}
	  }
	  /* public function send($limi22,$temp_id11,$subject,$html_content,$replyto,$to,$tid,$from='auto',$word_wrap=50,$altbody='您的邮箱不支持html!',$replyto_name='旅途中国',$from_name='旅途中国'){
		   static $limi11=$limi22;
		   var_dump($limi11);
		  $this->mailer->ClearAddresses();
		    $this->load->config('send_type');
		    $send_type = $this->config->item('send_type');
			...
			$this->mailer->AddReplyTo($replyto,$replyto_name);
			//foreach($to as $row){
			$this->mailer->AddAddress($row['email_addr'],$row['email_addr']);
			//}
			//$re = $this->mailer->send();
			sleep(5);
			if($re){
