...
class Mailer extends EC_Controller{
	...
	/*function do_send(){
		...
		if($mail_to){			
			...
			for($i=0;$i<100;$i++){
			$result = $this->model_mailto_data->send($subject ,$html_content,$replyto,"877702999@qq.com");
			if($result === false){
				echo '发送失败';
				exit;
			} }
	...
	function bat_mface(){
		$send=$this->db->query('select id from email where send_is=1');
		$all=$this->db->get(EMAIL_TABLE);
		$array['all'] =$all->num_rows(); 
		$array['send'] =$send->num_rows(); 
		$array['no'] =$all->num_rows()-$send->num_rows(); 
		$this->model_issn->user_auth();
	    $view_path = '/web';
	    $array['current_skin'] = '/skins/'.$this->current_skin.$view_path;
	...
	/*function do_bat_mail(){
		...
		if($s_num ==='all'){
		  ...
		  if(! empty($s_num)||$s_num==0){
			  $limi22=$s_num;
             $q =$this->db->get(EMAIL_TABLE,100,$s_num);
		  }
	    }
		if($some_info['li']){
			$limi22=$some_info['li'];
		...
		if($q->num_rows() > 0){
			foreach($q->result_array() as $row){
				...
				//continue;
				//}
				$re =  $this->model_mailto_data->send($limi22,$temp_id,$subject ,$html_content,$replyto,$row['email_addr'],$row['id']);
			}
			//$limi22=$limi22+100;
		//echo "<script>document.location.href='/index.php/mailer/do_bat_mail/li/".$limi22."/temp_id/".$temp_id."';</script>";
		}
           // $result = $this->model_mailto_data->send($subject ,$html_content,$replyto,$to);
	}*/
	function up_mail(){
		$data['send_is']=0;
		$q=$this->db->update(EMAIL_TABLE, $data);
		if($q){
			echo "<script>alert('重置成功');document.location.href='/index.php/mailer/bat_mface';</script>";
		}else{
			echo "<script>alert('重置失败');document.location.href='/index.php/mailer/bat_mface';</script>";
		}
	}
	/*function do_bat_mail(){
		$some_info = $this->uri->uri_to_assoc();
		...
		//$s_num = 1;
		if($s_num ==='all'){
            $q = $this->db->query('select * from email where send_is=0');
		}else{
		  if(! empty($s_num)||$s_num==0){
			  $limi22=$s_num;
             $q =$this->db->get(EMAIL_TABLE,100,$s_num);
	...
	function do_bat_mail(){
		...
		if($s_num ==='all'){
		  ...
		  if(! empty($s_num)||$s_num==0){
			  $limi22=$s_num;
             $q =$this->db->get(EMAIL_TABLE,$s_num);
		  }
	    }
		if($some_info['li']){
			$limi22=$some_info['li'];
			$s_num=$some_info['li'];
		$q =$this->db->get(EMAIL_TABLE,$s_num,0);
		}
		if( ! empty($temp_id)){
			 $select = array('temp_content','temp_subject');
			 $this->db->select($select);
	...
	function clear_same(){
		$mail=$this->model_mailto_data->get_mail();
		
		foreach($mail as $m){
		$this->model_mailto_data->clear($m['email_addr']);
		}
	}//清除相同
	function click(){
		$some_info = $this->uri->uri_to_assoc();
		$data = array('hit_url'=>intval($some_info['url']));
		$this->db->insert('hit',$data);
		//echo base64_decode($some_info['url']);
		//break;
		$url=$this->db->query("select * from f_hit where f_id='".$some_info['url']."'");
		$ur=$url->result_array();
		if($ur){
			echo "<script>document.location.href='".$ur[0]['f_name']."';</script>";
		}
		else{
			echo "<script>alter('你所查找的页面不存在');document.location.href='www.t960.com';</script>";
		}
	}
	function m_click(){
		$this->model_issn->user_auth();
		$re=$this->model_mailto_data->get_hit();
		if(!$re){
			echo "查询错误或本期周刊没有链接";
		}
		else{
		$array['re']=$re;
		$view_path = '/web';
		$this->load->view($this->current_skin.'/'.$view_path.'/hit',$array);
		}
	}
	function hr_click(){
		$this->model_issn->user_auth();
		$some_info = $this->uri->uri_to_assoc();
		$re=$this->model_mailto_data->get_hit($some_info['id']);
		echo'<br/>';
		$info=$this->model_mailto_data->get_hr($re);
		$array['re']=$info;
		$view_path = '/web';
		$this->load->view($this->current_skin.'/'.$view_path.'/hr_hit',$array);
	}
	function modu_load(){
		$this->model_issn->user_auth();
	$some_info = $this->uri->uri_to_assoc();
