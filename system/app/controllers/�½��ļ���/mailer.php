...
class Mailer extends EC_Controller{
	...
	function do_bat_mail(){
		...
		if($s_num ==='all'){
		  ...
		  if(! empty($s_num)||$s_num==0){
			  $limi22=$s_num;
             $q =$this->db->get(EMAIL_TABLE,20,$s_num*20);
		  }
	    }
		if($some_info['li']){
			$limi22=$some_info['li'];
	...
	/*function do_bat_mail(){
		...
		if($q->num_rows() > 0){
				...
				//continue;
				//}
				$re =  $this->model_mailto_data->send($limi22,$temp_id,$subject ,$html_content,$replyto,$q->result_array(),$row['id']);
